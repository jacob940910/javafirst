...
public class FileInputOutputStreamMain {
	...
	public static void main(String[] args) {
		//try - catch - finally는 각가의 블럭
		//파일에 바이트 단위로 기록할 수 있는 스트림변수 생성
		FileOutputStream fos = null;
		try {
			//예외가 발생할가능성이 있는 구문
			//연재 프로젝트 디렉토리에 파일을 생성
		...
		try {
			...
			*/
			...
			//4바이트씩 전체를 읽기
			//read에 바이트 배열을 대입하면 읽은개수를 리턴
			//0보다 작거나 같은 값을 리턴하면 읽을 데이터가 없는 것입니다.
			while(true) {
				byte [] b = new byte[100];
				int r = fis.read(b);
				//배열은 toString이 재정의 되어 있지않아서 해시코드가 출력
				//System.out.println(b);
